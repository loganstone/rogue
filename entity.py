...
class Entity:
    ...
    def move_astar(self, target, entities, game_map):
        ...
        # Scan all the objects to see
        # if there are objects that must be navigated around
        # Check also that the object isn't self or the target
        # (so that the start and the end points are free)
        # The AI class handles the situation
        # if self is next to the target
        ...
        # Compute the path between self's
        # coordinates and the target's coordinates
        libtcod.path_compute(my_path, self.x, self.y, target.x, target.y)

        # Check if the path exists, and in this case,
        # also the path is shorter than 25 tiles
        # The path size matters
        # if you want the monster to use alternative longer paths
        ...
        if not libtcod.path_is_empty(my_path) \
            ...
            # Keep the old move function as a backup so that
